...
inline float Sinewave::computeAt(const uint32_t idx) {
    return v_in_ * (float)sin(((double)idx/(double)sample_rate_hz_*(double)osc_frequency_*(double)detune_factor_)*PI*2.);
}
...
void Sinewave::SuperimposeNextSamples(float** out, uint32_t num_samples) {
    ...
    if (is_enabled_) {
        for (uint32_t i=0; i<num_samples; ++i) {
            float new_sample = v_in_ * (float)sin(((double)continuous_sample_index_/(double)sample_rate_hz_*(double)osc_frequency_*(double)detune_factor_)
                                                        *PI*2.);
            out[0][i] += new_sample;
            out[1][i] += new_sample;
